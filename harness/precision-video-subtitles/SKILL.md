...
- In Built-in Harness mode, source acquisition is deterministic infrastructure work: the runner resolves the local file or invokes the reviewed downloader, probes the resulting media, and updates `acquire` before asking the model for a controller action. Do not spend model turns rediscovering the source, guessing command syntax, or reading Harness/server source code.
- In Built-in Harness mode with local Faster-Whisper, full source transcription is deterministic infrastructure work too. After research passes, the runner creates output directories, extracts disk audio, builds a complete chunk inventory, transcribes only missing/invalid chunks, offsets timestamps, merges and validates real JSON, and optionally diarizes one bounded chunk at a time. The controller must not handcraft this FFmpeg/ASR/merge sequence or use a chat model to manufacture `source-transcript.json`.
- In Built-in Harness mode, translation orchestration is deterministic infrastructure work after research and source transcription pass. The runner reads the real `source-transcript.json`, groups stable cue IDs by the original disk chunk, sends bounded batches to the step-one model, validates every returned ID and non-empty translation, atomically preserves each completed sub-batch of at most 20 cues and each completed source chunk, merges `work/translated-subtitles.json`, registers it, and only then advances. The controller must not rediscover transcript chunks, copy transcript text into actions, probe JSON with media tools, invent a translation plan, or hand-merge model output.
- Treat malformed JSON, missing/unknown/duplicate IDs, and Unicode replacement characters as a failed translation sub-batch. Automatically bisect only that sub-batch and retry the smaller halves; preserve and reuse every already validated sub-batch.
- Translation batches use low/no-thinking mode when the provider supports it so the output budget is reserved for the JSON result. A successful HTTP response with empty content, a timeout, or a transient network error is recoverable at sub-batch scope and must shrink/retry; authentication, quota, and other explicit HTTP rejections remain blocking.
- A provider may use a smaller proven-safe request size inside the 20-cue atomic persistence unit (MiMo defaults to 10+10). Preserve the outer stable part filename and coverage so provider-specific safety tuning never invalidates completed checkpoint files.
- Never ask a model to read a local file merely by putting its path in the prompt. The Built-in Harness must parse the real translated cue data locally, deterministically classify ambiguity metadata, materialize `ambiguity-report.json` and `studio-review.json`, render SRT/ASS, run strict subtitle validation, mux, and run full stream/integrity verification. Models may receive only bounded real cue objects when semantic work is genuinely required; fabricated examples are invalid evidence.
- Every Built-in Harness controller reply is one complete JSON object. `run.args` is an array of separate argument strings, never a shell command string; `update_manifest` is a top-level tool, never a `run.id`. An empty model body, HTTP failure, timeout, invalid JSON, invalid schema, and tool failure are distinct errors and must remain distinct in logs and blockers.
- Prefer the Harness native `harness_action` function schema for controller turns and keep controller reasoning low; reserve the user's selected reasoning strength for bounded semantic work. The compatibility parser may translate only complete, allowlisted legacy XML tool blocks. Never repair arbitrary XML or guess a malformed JSON boundary.
- Keep controller context bounded and disk-backed. Retry a transient or empty model response once with compact context and low reasoning; retry an invalid action once with the exact schema correction. Only immediately adjacent failures with the same tool, arguments, and error count as consecutive; a successful or different tool call resets the counter. Two genuinely consecutive identical failures block the current phase with the real error and record path.
- Manifest phase names are restricted to the eight declared workflow phases, phase `evidence` is always an array, and stale unknown phases are removed during resume validation. Protocol retries are counted by exact error code; a different response-format failure must not masquerade as a repeated identical failure.
- Advance manifest phases in declared order. A later phase cannot become running, complete, blocked, or error while an earlier phase remains pending or running. A phase marked complete without a registered, readable required artifact is not resumable completion and must be reset with all downstream phases.
- Some OpenAI-compatible providers incorrectly encode the `calls` array inside `harness_action` as one JSON string. The Built-in Harness may decode that field only when it is a bounded, valid JSON array whose individual calls still pass the normal schema and tool allowlist; malformed strings, objects, prose, and oversized payloads remain protocol errors.
...
In Built-in Harness mode, perform that split with `materialize_research_preview` exactly once. Do not ask the controller model to echo the preview as nested `write_text` JSON. Large semantic outputs use `model_task.outputPath` or `model_batch.outputPath`; the controller receives only the path, byte count, usage, and bounded metadata.
...
For Built-in Harness local Faster-Whisper jobs, use the bundled `scripts/transcribe_media.py` pipeline rather than issuing individual FFmpeg or transcriber calls. Its merged output must parse as JSON, cover the full probed duration, contain every expected chunk, use globally continuous segment IDs and absolute timestamps, and preserve valid completed chunks on resume. A Markdown explanation, code sample, or model response saved with a `.json` suffix is never a transcript artifact.
...
In Built-in Harness jobs, this is executed by the runner. A restart must first reuse every valid `work/translation-parts/translated_chunk_NNN_part_NNN.json`, then every valid `work/translation-chunks/translated_chunk_NNN.json`, whenever cue IDs and source strings still match the real transcript; regenerate only the missing or invalid sub-batch/chunk. `translate` cannot become complete without a readable, fully validated `work/translated-subtitles.json` registered as `artifacts.translated_subtitles`.
...
- `scripts/transcribe_media.py`: deterministically extract, inventory, resume, merge, and validate full-media Faster-Whisper transcription.
