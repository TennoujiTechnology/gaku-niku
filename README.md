...
> 启动 GakuNiku 后，请优先完成第一步的模型测试。使用本地 Faster-Whisper 听写时，再让这个已验证模型检查并配置运行环境，然后再开始视频任务。

第一步通过文字与图片测试的模型，不只是后续的翻译模型，而是本次任务的统一执行模型。进入第二步后，点击“让第一步模型自动配置”，它会读取程序生成的脱敏环境诊断，选择最小且安全的配置方案：复用已有运行库和模型缓存，只补齐缺失或需要升级的项目。

AI 不会直接生成并执行任意 Shell 命令。项目内置 Harness 只允许它从固定白名单动作中选择，例如准备隔离 Python 环境、安装固定版本的听写依赖、下载所选模型，以及配置可选的说话人分离。任何依赖或模型下载都需要用户先确认下载内容、体积和保存位置。

- 本地听写：优先使用“让第一步模型自动配置”，不要先手动修改系统 Python。
- 在线听写 API：不需要安装本地 Faster-Whisper 环境。
- 已下载内容：重试时继续复用，不要求删除整个环境或重新下载完整模型。
- 配置失败：重新检查环境后，再让第一步模型根据最新诊断继续修复。
...
1. 设置并测试统一执行模型，这是后续 AI 环境配置的前提
2. 让已验证模型检查或配置听写环境，再选择视频、听写方案和输出格式
3. 检索并检查预习结果
4. 确认执行规则

任务开始后会依次执行素材获取、背景预习、原文听写、翻译、疑点复核、字幕质检、视频封装和最终验证。

开始任务后，同一个模型会继续负责检索规划、翻译、疑点复核和质量判断。API 模式不需要 Codex、Claude Code 或其他 Agent CLI，项目自带的 Harness 负责白名单文件、媒体和检索工具调用。
...
界面打开后，请先完成模型测试；选择本地听写时，优先点击“让第一步模型自动配置”完成环境检查与准备。
